<section class="section -bal pt-2 col_stretch bg-gray-50">
  <div class="row">
    <!-- basic cards -->
    <div class="col w-1/2@t w-1/3@d">
      <article class="card">
        <div class="card_body">
          <h2 class="card_title">Basic Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2@t w-1/3@d">
      <article class="card -bal">
        <a class="card_link" href="#">
          <div class="card_header">
            <div class="card_column">Header</div>
          </div>
          <div class="card_body">
            <h2 class="card_title">Basic Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Left</div>
            <div class="card_column">Footer Right</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2@t w-1/3@d">
      <article class="card bg-gray-800">
        <div class="card_body">
          <h2 class="card_title">Basic Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>

    <!-- image cap example -->
    <div class="col w-1/2@t w-1/3@d">
      <article class="card -ar-1x1 group">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Image Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2@t w-1/3@d">
      <article class="card">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_body">
            <h2 class="card_title">Image Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Left</div>
            <div class="card_column">Footer Right</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2@t w-1/3@d">
      <article class="card">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_header">
            <div class="card_column">Header</div>
          </div>
          <div class="card_body">
            <h2 class="card_title">Image Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Left</div>
            <div class="card_column">Footer Right</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2@t w-1/3@d">
      <article class="card bg-gray-800">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Image Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>

    <!-- image overlay example -->
    <div class="col w-1/2@t w-1/4@d">
      <article class="card -overlay">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Overlay Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
        <div class="card_footer">
          <div class="card_column">Footer Content</div>
        </div>
      </article>
    </div>
    <div class="col w-1/2@t w-1/4@d">
      <article class="card -overlay -middle">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_body">
            <h2 class="card_title">Overlay Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Content</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2@t w-1/4@d">
      <article class="card -overlay -bottom">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Overlay Card Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2@t w-1/4@d">
      <article class="card -overlay">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_body">
            <h2 class="card_title">Overlay Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Content</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2@t w-1/4@d">
      <article class="card -overlay">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_header">
            <div class="card_column">Header Content</div>
          </div>
          <div class="card_body">
            <h2 class="card_title">Overlay Card Title</h2>
            <p>Card content lorem ipsum</p>
          </div>
          <div class="card_footer">
            <div class="card_column">Footer Content</div>
          </div>
        </a>
      </article>
    </div>
  </div>

  <!-- orientation demo -->
  <div class="row card_zigzag">
    <div class="col w-1/2@t">
      <article class="card -h -v@d -h@w">
        <div class="card_media"></div>
        <div class="card_body">
          <div class="card_pretitle">Before the Title</div>
          <h2 class="card_title">Variable Orientation Demo</h2>
          <div class="card_subtitle">Subtitle is a bunch of lorem and other ipsum.</div>
          <p>Resize the viewport and watch me.</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2@t">
      <article class="card -h -v@d -h@w">
        <a class="card_link" href="#">
          <div class="card_media"></div>
          <div class="card_header">
            Header
          </div>
          <div class="card_body">
            <h2 class="card_title">Variable Orientation Demo</h2>
            <p>Resize the viewport and watch me.</p>
          </div>
          <div class="card_footer">
            Learn More
          </div>
        </a>
      </article>
    </div>
  </div>


  <!-- Profile -->

  <div class="row">
    <div class="col w-1/2 w-1/3@t w-20@d">
      <article class="card -profile group">
        <a href="#" class="card_link">
          <div class="card_media ar-1x1"></div>
          <div class="card_body">
            <h2 class="card_title">Kay Rooney</h2>
            <div class="card_subtitle">School of HummaJamma BoobaDooda, University of Edinburgh</div>
            <p>General Educational Development (GED) Examiner</p>
          </div>
          <div class="card_footer">
            <div class="card_column bwa-0 bwb-2 bc-none group-hover:bc-red"><i data-feather="more-vertical" class="-sm"></i>View Profile</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-20@d">
      <article class="card -profile group">
        <a href="#" class="card_link">
          <div class="card_media ar-1x1"></div>
          <div class="card_body">
            <h2 class="card_title">Cooper O'Neill</h2>
            <div class="card_subtitle">School of HummaJamma BoobaDooda, Boston College</div>
            <p>Individualized Education Plan (IEP) Aide</p>
          </div>
          <div class="card_footer">
            <div class="card_column bwa-0 bwb-2 bc-none group-hover:bc-red"><i data-feather="more-vertical" class="-sm"></i>View Profile</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-20@d">
      <article class="card -profile group">
        <a href="#" class="card_link">
          <div class="card_media ar-1x1"></div>
          <div class="card_body">
            <h2 class="card_title">Lillie Squires (Lil)</h2>
            <div class="card_subtitle">School of HummaJamma BoobaDooda, MIT</div>
            <p>Assistant Professor, Communication Studies and Faculty Affiliate, Women's', Gender, &amp; Sexuality Studies Program and Department of Cultures, Societies, &amp; Global Studies</p>
          </div>
          <div class="card_footer">
            <div class="card_column bwa-0 bwb-2 bc-none group-hover:bc-red"><i data-feather="more-vertical" class="-sm"></i>View Profile</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-20@d">
      <article class="card -profile group">
        <a href="#" class="card_link">
          <div class="card_media ar-1x1"></div>
          <div class="card_body">
            <h2 class="card_title">Millie Harper</h2>
            <div class="card_subtitle">School of HummaJamma BoobaDooda, Northeastern University</div>
            <p>Scholastic Aptitude Test (SAT) Grader</p>
          </div>
          <div class="card_footer">
            <div class="card_column bwa-0 bwb-2 bc-none group-hover:bc-red"><i data-feather="more-vertical" class="-sm"></i>View Profile</div>
          </div>
        </a>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-20@d">
      <article class="card -profile group">
        <a href="#" class="card_link">
          <div class="card_media ar-1x1"></div>
          <div class="card_body">
            <h2 class="card_title">Elizabeth Stuart (Liza)</h2>
            <div class="card_subtitle">College of Computer and Information Science, Northeastern University</div>
            <p>Associate Dean of Academic Affairs, Diversity and Inclusion and Dean's Professor of Civic Sustainability, College of Social Sciences and Humanities</p>
          </div>
          <div class="card_footer">
            <div class="card_column bwa-0 bwb-2 bc-none group-hover:bc-red"><i data-feather="more-vertical" class="-sm"></i>View Profile</div>
          </div>
        </a>
      </article>
    </div>
  </div>

  <!-- Tile Variant -->
  <div class="row">
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile">
        <div class="card_body">
          <h2 class="card_title">Default Tile Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile -middle">
        <div class="card_body">
          <h2 class="card_title">Middle Tile Title</h2>
          <p>Card content lorem ipsum. Maecenas sed diam eget risus varius blandit sit amet non magna.</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile -bottom">
        <div class="card_body">
          <h2 class="card_title">Bottom Tile Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile">
        <div class="card_body">
          <h2 class="card_title">Default Tile Title</h2>
          <p>Card content lorem ipsum. Maecenas sed diam eget risus varius blandit sit amet non magna.</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile -middle">
        <div class="card_body">
          <h2 class="card_title">Middle Tile Title</h2>
          <p>Card content lorem ipsum</p>
        </div>
      </article>
    </div>
    <div class="col w-1/2 w-1/3@t w-1/6@d">
      <article class="card -tile -bottom">
        <div class="card_body">
          <h2 class="card_title">Bottom Tile Title</h2>
          <p>Card content lorem ipsum. Maecenas sed diam eget risus varius blandit sit amet non magna.</p>
        </div>
      </article>
    </div>
  </div>
</section>

<!-- Flush variant -->
<section class="section -nogutters -flush py-0">
  <div class="row">
    <div class="col">
      <article class="card -h vh-30 bg-blue-dark">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Flush Card Title</h2>
          <p>Cards can be flush. Just add <code>-flush</code> and <code>-nogutters</code> options to its parent <code>.section</code>.</p>
        </div>
      </article>
    </div>
    <div class="col">
      <article class="card -h vh-30 bg-blue-dark -r">
        <div class="card_media"></div>
        <div class="card_body">
          <h2 class="card_title">Flush Card Title</h2>
          <p>Cards can be flush. Just add <code>-flush</code> to its parent <code>.section</code>.</p>
        </div>
      </article>
    </div>
  </div>
</section>

<!-- Stack wrapper -->
<section class="section">
  <div class="row">
    <div class="col w-1/3@d">
      <h3 class="fw-900">Stack of cards</h3>
      <div class="stack mb-1">
        <article class="card +avatar -h fs-xs">
          <a class="card_link items-center" href="#">
            <div class="card_media"></div>
            <div class="card_body pa-1">
              <h2 class="card_title">Stacked Horizontal Cards</h2>
              <p>These cards are wrapped in a Stack.</p>
            </div>
          </a>
        </article>
        <article class="card +avatar -h fs-xs">
          <a class="card_link items-center" href="#">
            <div class="card_media"></div>
            <div class="card_body pa-1">
              <h2 class="card_title">Stacked Horizontal Cards</h2>
              <p>These cards are wrapped in a Stack.</p>
            </div>
          </a>
        </article>
        <article class="card +avatar -h fs-xs">
          <a class="card_link items-center" href="#">
            <div class="card_media"></div>
            <div class="card_body pa-1">
              <h2 class="card_title">Stacked Horizontal Cards</h2>
              <p>These cards are wrapped in a Stack.</p>
            </div>
          </a>
        </article>
        <article class="card +avatar -h fs-xs">
          <a class="card_link items-center" href="#">
            <div class="card_media"></div>
            <div class="card_body pa-1">
              <h2 class="card_title">Stacked Horizontal Cards</h2>
              <p>These cards are wrapped in a Stack.</p>
            </div>
          </a>
        </article>
      </div>
    </div>

    <div class="col w-1/3@d">
      <h3 class="fw-900">Stack of articles</h3>
      <div class="stack">
        <article class="card fs-xs">
          <a class="card_link" href="#">
            <div class="card_body">
              <h2 class="card_title">Article Title</h2>
              <p>An excerpt would go here in the card body.
            </div>
            <div class="card_footer">
              By Mr. Metadata on March 14, 2020.
            </div>
          </a>
        </article>
        <article class="card fs-xs">
          <a class="card_link" href="#">
            <div class="card_body">
              <h2 class="card_title">Article Title</h2>
              <p>An excerpt would go here in the card body.
            </div>
            <div class="card_footer">
              By Mr. Metadata on March 14, 2020.
            </div>
          </a>
        </article>
        <article class="card fs-xs">
          <a class="card_link" href="#">
            <div class="card_body">
              <h2 class="card_title">Article Title</h2>
              <p>An excerpt would go here in the card body.
            </div>
            <div class="card_footer">
              By Mr. Metadata on March 14, 2020.
            </div>
          </a>
        </article>
        <article class="card fs-xs">
          <a class="card_link" href="#">
            <div class="card_body">
              <h2 class="card_title">Article Title</h2>
              <p>An excerpt would go here in the card body.
            </div>
            <div class="card_footer">
              By Mr. Metadata on March 14, 2020.
            </div>
          </a>
        </article>
      </div>
    </div>
  </div>
</section>
